...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, iscentered, isfloating, isurgent, neverfocus, oldstate, isfullscreen, cantfocus, issticky, isterminal, noswallow;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
drawbar(Monitor *m)
{
	...
	if (m == selmon || 1 /* YES */) { /* status is only drawn on selected monitor */
		/* drw_setscheme(drw, scheme[SchemeNorm]); */
